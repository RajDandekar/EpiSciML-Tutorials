...
Infected_Data = Array(sol)[2, :]
Recovered_Data = Array(sol)[3, :]

### Now we will construct a UDE (Universal Differential Equation). The simplest UDE we can construct is by replacing every interaction term in the SIRHD model with a neural network
##Define one neural network for each interaction term. We will use the Lux interface to define the neural networks.
p0_vec = []
...
##Match the optimized neural network prediction with the underlying data for infected and recovered
